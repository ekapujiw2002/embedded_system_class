...
// file var
FILE USARTInputOutputStream =
    FDEV_SETUP_STREAM(USART_OutputStream, USART_InputStream, _FDEV_SETUP_RW);

// usart init custom way
void USART_Init(const uint16_t aBaudRate, const uint8_t aChannel,
                const uint8_t aParity, const uint8_t aStopBits,
...
// for 1 channel usart
  ...
  /* set parity, stop bit, data bit format */
  UCSRC |=
      _BV(URSEL) | (PARITY_NONE << 4) | (STOP_BIT_1 << 3) | (DATA_BIT_8 << 1);
#endif
...
*/
void USART_Receive_String(uint8_t *RXBuffOut, const uint8_t max_len,
...
char USART_InputStream(FILE *stream) { return (char)USART_Receive(0); }

// usart set as stdin
void USART_SetStdin() { stdin = &USARTInputOutputStream; }

// usart set as stdout
void USART_SetStdout() { stdout = &USARTInputOutputStream; }
